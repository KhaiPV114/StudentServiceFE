...
const USER_ID = "6714c1a8f8e8b21b2f11a101";
...
const EventDetail = () => {
  ...
  const handleJoin = async () => {
    try {
      const res = await axios.post(`${API_URL}/${id}/join`, { userId: USER_ID });
      alert("✅ Bạn đã đăng ký sự kiện!");
      setEvent(res.data.event); // ✅ Cập nhật event mới từ API
    } catch (err) {
  ...
  const handleCancel = async () => {
    if (window.confirm("Bạn có chắc muốn hủy đăng ký sự kiện này không?")) {
      try {
        const res = await axios.post(`${API_URL}/${id}/leave`, { userId: USER_ID });
  ...
  const isJoined = event.participants?.includes(USER_ID);
      ...
      >
        ...
        <Card.Body>
          ...
          <p>
            ...
            <br />
            <strong>👥 Người tham dự:</strong> {participantsCount}/
            {event.capacity}
          </p>
